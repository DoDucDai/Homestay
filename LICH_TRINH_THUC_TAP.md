...
| **Đức Đại (BE)**| - Code xong API Login (`AuthController.java`).<br>- Code xong các API cơ bản (GET, POST) cho phòng ở `RoomController.java`. |
| **Leader** | - Hỗ trợ các bạn setup ban đầu.<br>- Review code của Sỹ Đại và Đức Đại.<br>- Viết Javascript (`auth.js`) để kết nối API Login của Đức Đại với giao diện `login.html`. |
...
| **Đức Đại (BE)**| - Hoàn thiện các API (PUT, DELETE) cho `RoomController.java`.<br>- Code toàn bộ các API (GET, POST, PUT, DELETE) cho `CustomerController.java` và `ServiceStatController.java` (phần Services). |
| **Leader** | - Viết Javascript (`rooms.js`) để gọi API của Đức Đại, hiển thị danh sách phòng, xử lý thêm/sửa/xóa phòng.<br>- Viết Javascript (`customers.js` và `services.js`) để hoàn thiện chức năng cho trang Khách hàng và Dịch vụ. |
...
| **Đức Đại (BE)**| - Code API Check-in (`PUT /api/checkin/{id}`) trong `BookingController.java`.<br>- Hỗ trợ Leader test các phần liên kết dữ liệu giữa các bảng. |
| **Leader** | - **(Quan trọng)** Code API Tạo Booking và API Check-out (tính tiền) trong `BookingController.java` (phần logic phức tạp).<br>- Viết Javascript (`bookings.js`, `checkin.js`, `checkout.js`) để gọi API, xử lý nghiệp vụ đặt phòng và trả phòng. |
...
| **Đức Đại (BE)**| - Dùng các câu query của Long và Hưng để viết API Thống kê (`ServiceStatController.java`).<br>- Sửa các lỗi lặt vặt (bugs) phát sinh khi test. |
| **Leader** | - Viết Javascript (`statistics.js`) gọi API để hiển thị số liệu thống kê lên màn hình.<br>- Ghép nối toàn bộ hệ thống, test end-to-end (từ lúc tạo khách hàng -> đặt phòng -> check out -> xem thống kê).<br>- Fix các bug khó. |
